...
[Django Uygulamasını Yayına Alma]
...
```
   adduser kullanici_adi  # adduser dijitalkale
```
   
2. Oluşturulan yeni kullanıcıya sudo (yönetici) yetkisi verme

```
   usermod -aG sudo kullanici_adi  # usermod -aG sudo dijitalkale
```
...
```
  git clone https://github.com/dijitalkale/eventhub.git
```
...
User=dijitalkale
Group=www-data
WorkingDirectory=/home/dijitalkale/eventhub
ExecStart=/home/dijitalkale/eventhub/venv/bin/gunicorn --access-logfile - --workers 3 --bind unix:/home/dijitalkale/eventhub/eventhub.sock eventhub.wsgi:application

[Install]
WantedBy=multi-user.target
```

**Not:** Sanal ortamın adı `venv`, projenin adı `eventhub`, kullanıcı adı `dijitalkale` olarak varsayılmıştır.
...
server {
    ...
    root /home/dijitalkale/eventhub; # Projenin kök dizini
    ...
    location / {
        ...
        proxy_pass http://unix:/home/dijitalkale/eventhub/eventhub.sock;  # Projenin kök dizinindeki 'proje_adı.sock' dosyası
    }
}
```
...
1. `settings.py` dosyasının bulunduğu dizinde (`/home/dijitalkale/eventhub/eventhub` klasörü içinde ) *settings* adında bir Python Paketi (içinde \_\_init\_\_.py dosyası bulunan bir klasör) oluşturulur.
...
server {
    listen 80;
    server_name alan_adınız www.alan_adınız; # server_name dijitalkale.com www.dijitalkale.com
...
server {
    ...
    root /home/kullanıcı_adı/proje_adı;  # root /home/dijitalkale/eventhub
    ...
    location / {
        include proxy_params;
        proxy_pass http://unix:/home/kullanıcı_adı/proje_adı/proje_adı.sock; # proxy_pass http://unix:/home/dijitalkale/eventhub/eventhub.sock;
